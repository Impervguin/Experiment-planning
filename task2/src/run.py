...
def run_pfe(ranges, task_count):
    ...
    def calc_regression(y, ranges):
        ...
        eq_lin_natural = f"y = {a0:.4f} + {a1:.4f}*λ1 + {a2:.4f}*λ2 + {a3:.4f}*σ0 + {a4:.4f}*σ1"
...
def run_pfe_gui():
    try:
        ranges = {
            ...
            "σ0": (dpg.get_value("s0_min"), dpg.get_value("s0_max")),
            "σ1": (dpg.get_value("s1_min"), dpg.get_value("s1_max")),
        }
...
def draw_table(tag, parent, data):
    ...
    with dpg.table(header_row=True, tag=tag, parent=parent, row_background=True):
        ...
        headers = ["x1","x2","x3","x4","λ1","λ2","σ0","σ1","y","y_lin","y_full","Δ_lin","Δ_full"]
        for h in headers:
...
def setup_page():
    with dpg.group(parent="main"):
        dpg.add_text("Полный факторный эксперимент (ПФЭ)")
        dpg.add_separator()
        ...
        dpg.add_input_float(label="σ0 (приоритет 0) min", tag="s0_min", default_value=1.7, width=300)
        dpg.add_input_float(label="σ0 (приоритет 0) max", tag="s0_max", default_value=2.1, width=300)

        dpg.add_input_float(label="σ1 (приоритет 1) min", tag="s1_min", default_value=1.5, width=300)
        dpg.add_input_float(label="σ1 (приоритет 1) max", tag="s1_max", default_value=2.5, width=300)

        dpg.add_input_int(label="Количество заявок", tag="task_count", default_value=1000)

        dpg.add_separator()

        dpg.add_button(label="Запустить ПФЭ", callback=run_pfe_gui, width=500, height=100)
        ...
        dpg.add_child_window(tag="table_group0", height=300, width=1800)

        dpg.add_separator()
        dpg.add_text("Приоритет 1")
        dpg.add_child_window(tag="table_group1", height=300, width=1800)
